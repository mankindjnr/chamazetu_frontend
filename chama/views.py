...
def refresh_token(request):
    try:
        ...
        refresh_access = requests.post(
            "https://sj76vr3h-9400.euw.devtunnels.ms/users/refresh",
...
def signin(request, role):
    if request.method == "POST":
        ...
        response = requests.post(
            "https://sj76vr3h-9400.euw.devtunnels.ms/users/login", data=data
        )
...
def signup(request, role):  # implement the manager signup
    if request.method == "POST":
        ...
        headers = {"Content-type": "application/json"}
        response = requests.post(
            "https://sj76vr3h-9400.euw.devtunnels.ms/users",
            data=json.dumps(data),
            headers=headers,
        )
